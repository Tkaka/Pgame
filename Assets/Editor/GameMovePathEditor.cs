...
[CustomEditor(typeof(GameMovePath))]
internal sealed class GameMovePathEditor : BaseEditor<GameMovePath>
{
    ...
    void DrawWaypointEditor(Rect rect, int index)
    {
        ...
        GameMovePath.GameWaypoint def = new GameMovePath.GameWaypoint();
        ...
        if (GUI.Button(r, setButtonContent, GUI.skin.label))
        {
            ...
            GameMovePath.GameWaypoint wp = Target.m_Waypoints[index];
        ...
        EditorGUI.LabelField(r, "Zone");

        r.x += hSpace + r.width;
        r.width = rect.width - (textDimension.x + hSpace + r.width + hSpace + addButtonDimension.x + hSpace);
        EditorGUI.PropertyField(r, element.FindPropertyRelative(() => def.zoneId), GUIContent.none, true);
    }
    ...
    void InsertWaypointAtIndex(int indexA)
    {
        ...
        var wp = new GameMovePath.GameWaypoint();
        wp.position = pos;
        wp.roll = roll;
        var list = new List<GameMovePath.GameWaypoint>(Target.m_Waypoints);
    ...
    {
        GameMovePath.GameWaypoint wp = Target.m_Waypoints[i];
        EditorGUI.BeginChangeCheck();
    ...
    [DrawGizmo(GizmoType.Active | GizmoType.NotInSelectionHierarchy
         | GizmoType.InSelectionHierarchy | GizmoType.Pickable, typeof(GameMovePath))]
    static void DrawGizmos(GameMovePath path, GizmoType selectionType)
    {
        DrawPathGizmo(path,
    ...
    internal static void DrawPathGizmo(GameMovePath path, Color pathColor)
    {
        // Draw the path

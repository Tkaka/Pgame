...
Shader "DGame/SNpc_MatCap_OutLine" {
    Properties {
        _Hit ("Hit", Float ) = 1
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _SRAMap; uniform float4 _SRAMap_ST;
...
////// Lighting:
                ...
                half4 SRA = tex2D(_SRAMap,TRANSFORM_TEX(i.uv0.rg, _SRAMap));
                clip(saturate(SRA.b) - 0.5);
				fixed4 amc = tex2D(_MatCap1, i.cap);
				fixed4 mc = tex2D(_MatCap, i.cap)*_ReflectPower;
                half LightFallOff = (SRA.r*amc.r);
                //half StepFallOff = floor(LightFallOff * _FallOffstep);
                ...
                half3 CombinColor = lerp(CombinC,CombinC+(mc*2.0)-1.0,saturate(SRA.g - _LerpS)) ;
...
/// Final Color:
                return fixed4(finalColor,saturate(SRA.b-_Alphabias));
            }
            ENDCG
        }
               Pass {
           ...
           // uniform fixed4 _LightColor0;
            ...
            VertexOutput vert (VertexInput v) {
                VertexOutput o;
                 o.uv0 = v.texcoord0;
                //float4 SRA = tex2Dlod(_SRAMap,float4(TRANSFORM_TEX(o.uv0 .rg, _SRAMap),0.0,0));
                //o.pos = mul(UNITY_MATRIX_MVP, float4(v.vertex.xyz + v.normal*(_OutWidth*0.00285*SRA.g),1));
	...
	return float4( 0.8 * col.rgb * col.rgb* _OutLineColor  * _OutLinePow, saturate(SRA.b-_Alphabias) )  ; 
            }
            ENDCG
        }
    }

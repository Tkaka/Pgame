...
Shader "PGame/ActorOptSimple_Mask" {
    Properties {
        _Hit ("Hit", Float ) = 1
...
////// Lighting:
				...
				fixed4 mc = _ReflectPower;
                //half LightFallOff = (SAG.r*2*max(0,dot(normalDirection,lightDirection)));
				half LightFallOff = 2 * max(0, dot(normalDirection, lightDirection));
				...
				if (_IsHit)
				{
					_HitColor = half4(1, 0.815, 0.815, 1);
					Diff = half4(1, 1, 1, 1) * (LightFallOff + _HitColor);
					CombinC = Diff.rgb*_MainColor.rgb;
				}
				else
				{
					CombinC = Diff.rgb*_MainColor.rgb*saturate(lerp((If_LeA*If_Output) + (If_LeB*1.0), If_Output, If_LeA*If_LeB))*_Hit;
				}
				...
				half3 CombinColor = lerp(CombinC, CombinC + (mc*2.0) - 1.0, 1-_LerpS);
...
/// Final Color:
        ...
        /*Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
	            ...
	            return float4( 0.8 * Diff.rgb * Diff.rgb* _OutLineColor  * _OutLinePow, saturate(SAG.g-_Alphabias) )  ; 
            }
            ENDCG
        }*/
    }

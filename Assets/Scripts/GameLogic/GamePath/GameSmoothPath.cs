...
//[SaveDuringPlay]
public class GameSmoothPath : GamePathBase
{
    ...
    private void ResetSpawnerPos()
    {
        foreach (GameWaypoint wp in m_Waypoints)
        {
            if (wp.spawner != null)
            {
                wp.spawner.transform.localPosition = wp.position;
            }
        }
    }

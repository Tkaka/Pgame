...
namespace FastShadowReceiver {
	...
	public class AutoProjector : MonoBehaviour {
		...
		public bool UpdateVisibility(Plane zPlane, Plane[] cameraClipPlanes, float cameraNear, float cameraFar)
		{
			...
			m_isVisible = false;
			for (int i = 0; i < cameraClipPlanes.Length; ++i) {
				float distance = cameraClipPlanes[i].GetDistanceToPoint(o);
				float dot = Vector3.Dot(cameraClipPlanes[i].normal, z);
				if (distance * dot <= 0) {
					Plane targetPlane = cameraClipPlanes[i];
					if (distance < 0) {
						targetPlane.normal = -targetPlane.normal;
						targetPlane.distance = -targetPlane.distance;

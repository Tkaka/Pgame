...
Shader "FastShadowReceiver/Projector/Light x EnhancedShadow" {
	Properties {
		_Color ("Main Color", Color) = (1,1,1,1)
		_LightTex ("Cookie", 2D) = "" {}
		_FalloffTex ("FallOff", 2D) = "" {}
		_Offset ("Offset", Range (-1, -10)) = -1.0
		_ShadowEnhance ("Shadow Enhance", Range (0, 1)) = 0
	}
	...
	Subshader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : COLOR
			{
				...
				fixed enhance = 0;
				#if 0 < FSR_SHADOW_NUM
				fixed4 shadowTex = tex2Dproj(_ShadowTex1, UNITY_PROJ_COORD(i.uvShadow1));
				enhance += _ShadowEnhance * shadowTex.a;
				col.rgb *= lerp(fixed3(1,1,1), shadowTex.rgb, saturate(i.uvShadow1.z));
				#endif
				#if 1 < FSR_SHADOW_NUM
				shadowTex = tex2Dproj(_ShadowTex2, UNITY_PROJ_COORD(i.uvShadow2));
				enhance += _ShadowEnhance * shadowTex.a;
				col.rgb *= lerp(fixed3(1,1,1), shadowTex.rgb, saturate(i.uvShadow2.z));
				#endif
				#if 2 < FSR_SHADOW_NUM
				shadowTex = tex2Dproj(_ShadowTex3, UNITY_PROJ_COORD(i.uvShadow3));
				enhance += _ShadowEnhance * shadowTex.a;
				col.rgb *= lerp(fixed3(1,1,1), tex2Dproj(_ShadowTex3, shadowTex.rgb, saturate(i.uvShadow3.z));
				#endif

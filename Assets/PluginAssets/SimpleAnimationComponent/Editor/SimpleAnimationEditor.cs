...
public class SimpleAnimationEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
		...
		if (GUILayout.Button("预设动画名"))
        {
            string[] arr = { "idle", "attack", "dead", "fukong", "hurt", "jifei", "move", "skill1", "skill2", "xuanyun", "jidao", "chuchang", "idle" };
            states.arraySize = arr.Length;
            for (int i = 1; i < arr.Length; ++i)
            {
                var obj = states.GetArrayElementAtIndex(i);
                var name = obj.FindPropertyRelative("name");
                name.stringValue = arr[i];
                if (clip.objectReferenceValue != null)
                {
                    string path = AssetDatabase.GetAssetPath(clip.objectReferenceValue);
                    string prePath = path.Substring(0, path.Length - System.IO.Path.GetFileName(path).Length);
                    var aniClip = obj.FindPropertyRelative("clip");
                    aniClip.objectReferenceValue = AssetDatabase.LoadAssetAtPath<AnimationClip>(prePath + arr[i] + ".anim");
                }
            }
        }
